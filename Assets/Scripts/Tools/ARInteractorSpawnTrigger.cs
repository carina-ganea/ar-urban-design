...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.ARStarterAssets
{
    ...
    public class ARInteractorSpawnTrigger : MonoBehaviour
    {
        ...
        [SerializeField] XRRayInteractor m_XRInteractor;
        ...
        void Update()
        {
            ...
            {
                var obj = m_ARInteractorAsControllerInteractor.interactablesSelected[0].transform;

                if(obj.GetComponent<NetworkObject>() != null)
                {
                    var objectId = obj.GetComponent<NetworkObject>().NetworkObjectId;
            ...
            if (attemptSpawn && m_ARInteractor.TryGetCurrentARRaycastHit(out var arRaycastHit))
            {
                ...
                m_ObjectSpawner.TrySpawnGround(arRaycastHit.pose.position, arPlane.normal);
            }

            if( attemptSpawn && m_XRInteractor.TryGetCurrent3DRaycastHit(out var raycastHit) && raycastHit.transform.tag == "Ground")
            {
                // Only spawn when the touch begins
                Touch touch;
                if (Input.touchCount >= 1 && (touch = Input.GetTouch(0)).phase != TouchPhase.Began)
                ...
                m_ObjectSpawner.TrySpawnObject(raycastHit.point, raycastHit.normal);
            }
        }
    }
}